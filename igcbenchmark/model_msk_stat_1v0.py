...
from torchutils import (
    ...
    init_linear, set_dtld_seed, set_worker_seed, fix_cpu_affinity)
from igc import (
...
class Dataset:
    def __init__(self, imgstat_set_name, img_size=64, img_aug=False,
                 imst_aug_std=None, val_ratio=0.1, seed=100):
        self.img_size = img_size
        # Load data
        imst_set = ImgStatSet(imgstat_set_name).load()
        self.masks = imst_set.get_masks(self.img_size)
        img_set = imst_set.get_img_set(self.img_size)
        # Standardize data
        img_set.standardize()
        imst_set.standardize()
        # Training and validation indices
        self.val_idx, self.train_idx = img_set.get_random_split(
            val_ratio, seed)
        ...
        self.img_set_val = img_set.select(self.val_idx)
        self.img_set_train = img_set.select(self.train_idx)
        self.imst_set_val = imst_set.select(self.val_idx)
        self.imst_set_train = imst_set.select(self.train_idx)
        del img_set, imst_set
        # Compute augmentation values
        if img_aug:
            self.img_set_train.augment()
        self.imst_set_train.compute_augmentation_values(imst_aug_std)

    @torch.no_grad()
    def train_dtld(self, batch_size, seed=100, num_workers=0):
        return DataLoader(
            _Dataset(self.img_set_train, self.imst_set_train, seed),
    ...
    def val_dtld(self, batch_size, seed=None, num_workers=0):
        if seed is None:
            return DataLoader(
                _Dataset(self.img_set_val, self.imst_set_val),
    ...
    def get_masks(self):
        masks = tuple(
            mask.mask.mean(axis=2, keepdims=True) for mask in self.masks)
        return np.concatenate(masks, axis=2).transpose((2, 0, 1))
...
class Encoder(nn.Module):
    def __init__(self, img_size, n_imst, conv_stem_kernel, conv_sizes,
        ...
        # Output layer
        self.lin_out = nn.Linear(lin_sizes[-1], n_imst, bias=True)
    ...
    def r_loss(self, y_r, y):
        loss_per_imst = self.mse_loss(y_r, y).sum(dim=0)
...
class Model(AbstractModel):
    ...
    def __init__(self, dataset, model_name='msk_stat_1v0', trainable=False,
                 device=None, parameters=None):
        self.dtst = dataset
        super().__init__(model_name, device, parameters)
        # Network
        self.network = Encoder(
            self.dtst.img_size, self.dtst.imst_set_val.n_imst,
    ...
    def train(self, n_epoch=1, batch_size=64, num_workers=0):
        ...
        for _ in range(int(n_epoch)):
            ...
            for i, (x, y) in enumerate(tqdm(
                ...
                y_r = self.network(x)
                r_loss, _ = self.network.r_loss(y_r, y)
                r_loss.backward()
                self.optimizer.step()
                # Summary
                train_r_avg += r_loss.item()
            train_r_avg /= float(len(train_dtld) * batch_size)
            writer.add_scalar('train/r_loss', train_r_avg, self.current_epoch)
            # Validation ######################################################
            self.network.eval()
            with torch.no_grad():
                val_r_avg = 0.0
                for i, (x, y) in enumerate(tqdm(
                    ...
                    r_loss, _ = self.network.r_loss(y_r, y)
                    # Summary
                    val_r_avg += r_loss.item()
            val_r_avg /= float(self.dtst.n_val)
            writer.add_scalar(
                'validation/r_loss', val_r_avg, self.current_epoch)
            # Save checkpoint #################################################
    ...
    def score(self, batch_size=64, checkpoint='best'):
        ...
        val_corr = np.zeros(self.dtst.imst_set_val.n_imst, dtype=np.float32)
        for i in range(self.dtst.imst_set_val.n_imst):
    ...
    def _igc_params(self, checkpoint, num_workers=0):
        ...
        y_size = self.dtst.imst_set_val.n_imst
